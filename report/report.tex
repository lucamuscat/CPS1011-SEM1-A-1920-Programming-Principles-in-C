...
\clearpage
  ...
  \begin{longtable}{l p{10cm}}
    ...
    \textbf{Line 87} & char array of size greater than 55 will just call encode_long_object with an offset of 0xb7. \\ \\
    \textbf{Line 89} & Concatenate the message with the encoding. \\ \\
    \textbf{Line 92-103} & Identical to lines 73-90 but with different offsets. \\ \\
  ...
  \subsubsection{sendMessage}
   ...
   \begin{longtable}{l p{10cm}}
     \textbf{Line 1} & sendMessage requires a message queue, ID to send message to (which can be null to signify that sendMessageBatch will be used instead), sender, subject and content  \\

     \textbf{Line 5-7} & If the ID provided is NULL, pass the arguments into the sendMessageBatch method. This will return a success status.\\

     \textbf{Line 9} & Call the recursive method populate\_individual\_node.  \\
  ...
  \clearpage
   ...
   \begin{longtable}{l p{10cm}}
     \textbf{Line 4-5} & Check if the provided node is NULL (meaning that the createQ method hasn't been called yet), if it is NULL, return a failure status. \\

     \textbf{Line 8} & Check if the node's ID matches the specified identifier.\\
